...
1. 运行 `Translator-Setup-v1.1.5.exe`。
...
自动翻译和手动翻译走同一条请求链。免费词典暂时错误时，一次操作内部最多重试三次并缓存成功结果；持续失败时自动降级为 Chrome 基础翻译，并明确提示本次可能缺少音标和扩展释义。浏览器翻译页面的通信端口失活时，扩展会重建页面并自动重发一次请求。
